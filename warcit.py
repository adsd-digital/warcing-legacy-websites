# Adapted module from https://github.com/webrecorder/warcit/blob/help-text-slash/warcit/warcit.py
# Warcit
# Copyright 2017-2020 Webrecorder Software LLC, Rhizome, and Contributors.
# Authored by Ilya Kreymer
#
# Small change: added import and methods for usage of html_transformer.py
...
from warcit.html_transformer import TransformationSerializer#, TransclusionSerializer
...
class WARCIT(BaseTool):
    def __init__(self, url_prefix, inputs,
        ...
        if transformations:
            self.transformation_serializer = TransformationSerializer(transformations)
        else:
            self.transformation_serializer = None
    ...
    def run(self):
        ...
        with closing(output):
            ...
            for file_info in self.iter_inputs():
                ...
                if self.transformation_serializer:
                    self.make_transformations(writer, url, record)
    ...
    def make_transformations(self, writer, url, record):
        for file_info, type_, metadata in self.transformation_serializer.find_transformations(url):
            extra_headers = {'WARC-Refers-To': record.rec_headers['WARC-Record-ID'],
                             'WARC-Refers-To-Target-URI': record.rec_headers['WARC-Target-URI'],
                             'WARC-Refers-To-Target-Date': record.rec_headers['WARC-Date']
                            }
